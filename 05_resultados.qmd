...
## Cenário geral da fiscalização eletrônica de velocidade
...
## Indicadores brasileiros sobre fiscalização eletrônica de velocidade – Unidades da Federação
...
gtsave(tabela_ufs, filename = "export/tabela_ufs.png")
...
Nos gráficos da @fig-serie-i1 à @fig-serie-i7 podem ser consultadas as séries temporais no período 2023 a 2026 para os indicadores de fiscalização eletrônica de velocidade $IF_1$, $IF_2$, $IF_6$ e $IF_7$.
...
## Indicadores brasileiros sobre fiscalização eletrônica de velocidade – Municípios

Na @tbl-municipios-i1, @tbl-municipios-i2, @tbl-municipios-i3, @tbl-municipios-i4, são apresentados os indicadores de fiscalização eletrônica de velocidade $IF_1$ ao $IF_4$ para os municípios brasileiros. Os valores são apresentados em ordem decrescente de cada indicador para os 10 municípios com os maiores valores.
...
#| fig-cap: "Ranking de municípios segundo o $IF_1$"
if1 <- read.csv("export/2024_municipios_if1.csv") %>%
...
gtsave(data = if1, filename = "export/if1_municipios.png")
...
#| fig-cap: "Ranking de municípios segundo o $IF_2$"
if2 <- read.csv("export/2024_municipios_if2.csv") %>% 
...
gtsave(if2, "export/if2_municipios.png")
if2
```
...
#| fig-cap: "Ranking de municípios segundo o $IF_3$"
if3 <- read.csv("export/2024_municipios_if3.csv") %>% 
...
gtsave(if3, "export/if3_municipios.png")
...
#| fig-cap: "Ranking de municípios segundo o $IF_4$"
if4 <- read.csv("export/2024_municipios_if4.csv") %>% 
...
gtsave(if4, "export/if4_municipios.png")
...
gtsave(tabela_capitais, filename = "export/tabela_capitais.png")
...
#| 
grafico <- read_csv("export/uf_tabela_correlacao_2024.csv") %>% 
...
ggsave(filename = "export/grafico_correlacao_relatorio.png", plot = grafico, )
grafico
```
