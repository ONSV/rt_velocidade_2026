# Método {#sec-metodologia}

Os dados sobre fiscalização eletrônica de velocidade foram extraídos dos relatórios de verificação do Instituto Nacional de Metrologia, Qualidade e Tecnologia (INMETRO), disponíveis por meio do Portal de Serviços do Inmetro nos Estados - PSIE e do Portal de Dados Abertos do Governo Federal [@brasil2024b_inmetro]. A metodologia de extração dos dados foi desenvolvida pela Puerta Engenharia [@puerta2024]. O presente estudo foi baseado em estudo anterior [@onsv_ufpr2025_indicadores], porém contou com novos procedimentos metodológicos a fim de melhor explorar os dados disponíveis.
...
Para o cálculo dos indicadores, utilizou-se os dados de frota de veículos disponibilizados pela Secretaria Nacional de Trânsito [@brasil2025a_senatran], a quantidade de quilômetros percorridos estimada em @onsv_ufpr2026_taxa e a extensão de rodovias federais disponibilizada pelo Departamento Nacional de Infraestrutura de Transportes [@brasil2024c_dnit].
...
A fim de obter um diagnóstico das capitais brasileiras quanto ao nível de fiscalização eletrônica de velocidade, foi calculado um oitavo indicador ($IF_8$), correspondente ao número de faixas fiscalizadas por câmeras fixas de segurança a cada 100 quilômetros de vias do município. A extensão das vias foi obtida a partir de @osm2024.

No nível estadual, foram compostos dois indicadores sobre infrações relacionadas à velocidade 
($II_1$ e $II_2$), considerando apenas aquelas infrações detectadas por dispositivos eletrônicos de medição de velocidade (câmeras de segurança), ou seja, as infrações dos códigos **7455**, **7463** e **7471**, seja o medidor fixo, estático, móvel ou portátil. O ano de referência adotado foi de 2024. Foram utilizados os dados de infrações disponibilizados pela Secretaria Nacional de Trânsito [@brasil2025b_infracoes]. Os indicadores sobre infrações relacionadas à velocidade encontram-se listados a seguir:
...
-   **Indicador 2** ($II_2$) – Número de infrações de velocidade por câmera de segurança, manifestando o nível de obediência à fiscalização eletrônica.
...
Na @tbl-infracoes podem ser consultados os valores considerados para II1, II2 e IM1.

```{r}
#| label: tbl-infracoes
#| tbl-cap: "Indicadores de infrações e mortalidade para Unidades da Federação"
...
tabela_infracoes <- read.csv("export/tabela_infracoes.csv") %>% 
  ...
  fmt_number(columns = c(ii1,ii2, taxa_2024), decimals = 2, dec_mark = ",",
    sep_mark = ".") %>% 
  cols_width(
      uf ~ px(20), 
      taxa_2024 ~ px(20),
...
gtsave(tabela_infracoes, filename = "export/tabela_infracoes.png")
